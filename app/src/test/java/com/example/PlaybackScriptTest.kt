...
class PlaybackScriptTest {
    ...
    @Test
    fun `script guards screen-off playback with user-pause intent`() {
        val script = PlaybackScript.build(generation = 19, resumePositionSeconds = 0.0)

        assertTrue(script.contains("__cluBackgroundPlayEnabled"))
        assertTrue(script.contains("__cluUserPaused"))
        assertTrue(script.contains("visibilitychange"))
        assertTrue(script.contains("maybeResumeAfterHide"))
    }

    @Test
    fun `script honors background play disabled on fresh page load`() {
        val script = PlaybackScript.build(
            generation = 20,
        ...
        assertTrue(script.contains("__cluBackgroundPlayEnabled = false"))
    }
}
