...
class ReleaseNotificationWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            ...
            if (ReleaseNotificationScheduler.alertId(inputData) == null) {
                ...
                deliverDueAlerts(context, store)
                reconcilePendingAlerts(context, store)
            }
            Result.success()
        } catch (_: Exception) {
            Result.retry()
        ...
        store.getReleaseAlerts()
            ...
            .forEach { alert ->
                ...
                val notification = AppNotification(
                    ...
                    message = if (alert.kind == NotificationKind.WATCHED_SHOW_EPISODE &&
                        alert.season != null && alert.episode != null
                    ) {
                        "S${alert.season} · E${alert.episode} of ${target.title} is ready to watch."
                    } else if (alert.episode != null) {
    ...
    /**
     * Bell reliability net: one-shot timers can be dropped by battery
     * optimization or a cleared work queue. On every periodic run, re-register
     * the timer for alerts going off within the next 24h (REPLACE makes this
     * idempotent), so a lost timer self-heals instead of a missed bell.
     */
    private suspend fun reconcilePendingAlerts(context: Context, store: LocalStore) {
        val now = System.currentTimeMillis()
        val horizon = now + java.util.concurrent.TimeUnit.HOURS.toMillis(24)
        store.getReleaseAlerts()
            .filter { !it.isDelivered && it.releaseAtMillis > now && it.releaseAtMillis <= horizon }
            .forEach { ReleaseNotificationScheduler.scheduleAlert(context, it) }
    }
    ...
    private suspend fun syncSubscriptions(store: LocalStore) {
        ...
        settings.subscribedChannelNames.forEach { channelName ->
            ...
            current.filterNot { it.id in previousIds }
                // Catalog order shifts as titles trend: only genuinely fresh
                // releases ping. A 2005 title surfacing in the list is
                // backfill, not a "New from ..." event.
                .filter { isFreshRelease(it.releaseDateIso ?: it.releaseDateFormatted, now) }
                .forEach { video -> if (settings.watchedVideoIds.none { it == video.id }) {
                    val notification = AppNotification(
