...
class SettingsManager(context: Context) {
    ...
    /**
     * Background play: when true, audio keeps playing when the screen turns
     * off or the app goes to the background (foreground service + wake lock).
     * When false, playback pauses on background (legacy behavior).
     */
    ...
    var watchLaterSort: com.example.model.WatchLaterSort
        get() = runCatching {
            com.example.model.WatchLaterSort.valueOf(
                prefs.getString(KEY_WATCH_LATER_SORT, null) ?: ""
            )
        }.getOrDefault(com.example.model.WatchLaterSort.RECENTLY_ADDED)
        set(value) = prefs.edit().putString(KEY_WATCH_LATER_SORT, value.name).apply()

    /** Persisted layout override (You tab / Settings). Unknown values fall back to MOBILE. */
    var deviceLayoutMode: com.example.model.DeviceLayoutMode
        get() = runCatching {
            com.example.model.DeviceLayoutMode.valueOf(
                prefs.getString(KEY_DEVICE_LAYOUT_MODE, null) ?: ""
            )
        }.getOrDefault(com.example.model.DeviceLayoutMode.MOBILE)
        set(value) = prefs.edit().putString(KEY_DEVICE_LAYOUT_MODE, value.name).apply()
    ...
    /**
     * When true, an active skip pill auto-fires after [skipAutoSkipDelaySeconds]
     * (unless its type opted out below). Tapping the player (any touch outside
     * the pill) cancels the pending auto-skip and hides the pill for that segment.
     */
    var isSkipAutoSkipEnabled: Boolean
        get() = prefs.getBoolean(KEY_SKIP_AUTO_SKIP_ENABLED, true)
        set(value) = prefs.edit().putBoolean(KEY_SKIP_AUTO_SKIP_ENABLED, value).apply()

    /**
     * Auto-skip countdown in seconds, 0..[SKIP_AUTO_SKIP_DELAY_MAX_SECONDS].
     * 0 disables auto-skip (button only) regardless of the master toggle.
     */
    var skipAutoSkipDelaySeconds: Int
        get() = prefs.getInt(
            KEY_SKIP_AUTO_SKIP_DELAY_SECONDS,
            SKIP_AUTO_SKIP_DELAY_DEFAULT_SECONDS
        ).coerceIn(0, SKIP_AUTO_SKIP_DELAY_MAX_SECONDS)
        set(value) = prefs.edit().putInt(
            KEY_SKIP_AUTO_SKIP_DELAY_SECONDS,
            value.coerceIn(0, SKIP_AUTO_SKIP_DELAY_MAX_SECONDS)
        ).apply()

    /** Per-type auto-skip opt-ins. Visibility toggles above still gate the pill itself. */
    var isSkipAutoSkipIntroEnabled: Boolean
        get() = prefs.getBoolean(KEY_SKIP_AUTO_SKIP_INTRO_ENABLED, false)
        set(value) = prefs.edit().putBoolean(KEY_SKIP_AUTO_SKIP_INTRO_ENABLED, value).apply()

    var isSkipAutoSkipRecapEnabled: Boolean
        get() = prefs.getBoolean(KEY_SKIP_AUTO_SKIP_RECAP_ENABLED, false)
        set(value) = prefs.edit().putBoolean(KEY_SKIP_AUTO_SKIP_RECAP_ENABLED, value).apply()

    var isSkipAutoSkipCreditsEnabled: Boolean
        get() = prefs.getBoolean(KEY_SKIP_AUTO_SKIP_CREDITS_ENABLED, true)
        set(value) = prefs.edit().putBoolean(KEY_SKIP_AUTO_SKIP_CREDITS_ENABLED, value).apply()

    var isSkipAutoSkipPreviewEnabled: Boolean
        get() = prefs.getBoolean(KEY_SKIP_AUTO_SKIP_PREVIEW_ENABLED, true)
        set(value) = prefs.edit().putBoolean(KEY_SKIP_AUTO_SKIP_PREVIEW_ENABLED, value).apply()
    ...
    companion object {
        ...
        private const val KEY_SKIP_AUTO_SKIP_ENABLED = "skip_auto_skip_enabled"
        private const val KEY_SKIP_AUTO_SKIP_DELAY_SECONDS = "skip_auto_skip_delay_seconds"
        const val SKIP_AUTO_SKIP_DELAY_DEFAULT_SECONDS = 4
        const val SKIP_AUTO_SKIP_DELAY_MAX_SECONDS = 15
        private const val KEY_SKIP_AUTO_SKIP_INTRO_ENABLED = "skip_auto_skip_intro_enabled"
        private const val KEY_SKIP_AUTO_SKIP_RECAP_ENABLED = "skip_auto_skip_recap_enabled"
        private const val KEY_SKIP_AUTO_SKIP_CREDITS_ENABLED = "skip_auto_skip_credits_enabled"
        private const val KEY_SKIP_AUTO_SKIP_PREVIEW_ENABLED = "skip_auto_skip_preview_enabled"
