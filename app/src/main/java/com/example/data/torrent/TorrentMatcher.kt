...
object TorrentMatcher {
    ...
    fun matchEpisode(releaseName: String, season: Int, episode: Int): EpisodeMatchResult {
        ...
        // 4. Multi-episode ranges or complete packs (must check before mentionsOtherEpisode
        // so "S01E01-E08" is classified as MULTI instead of rejected as WRONG).
        if (isMultiEpisodeRange(name, s)) {
            return EpisodeMatchResult(EpisodeMatch.MULTI, isPack = true)
        }
        ...
        // 5. Mentions a DIFFERENT episode of the same/different season -> WRONG.
