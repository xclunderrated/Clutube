        ...
        val script = """
            (function() {
                ...
                if (window.MutationObserver && !window.__cluAutoplayObserver) {
                    // Throttled to 1s: the unthrottled subtree observer ran
                    // enableAutoplay/pin/tryAutoPlay on every DOM mutation
                    // (VidLink video.js churn) — a steady jank source.
                    var __cluAdblockLast = 0;
                    window.__cluAutoplayObserver = new MutationObserver(function() {
                        var now = Date.now();
                        if (now - __cluAdblockLast < 1000) return;
                        __cluAdblockLast = now;
                        enableAutoplayOnFrames();
                        pinTopActions();
                        tryAutoPlay();
                    });
