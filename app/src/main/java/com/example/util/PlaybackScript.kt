    ...
    fun build(
        ...
        return """
            (function() {
                ...
                // Background play: authoritative value comes from Kotlin
                // (Settings toggle via PlayerViewManager) on every page load,
                // so a fresh document can never re-enable what the user
                ...
                function hookVideo(video) {
                    ...
                    ['loadedmetadata', 'durationchange', 'canplay', 'timeupdate', 'play', 'pause', 'seeked', 'volumechange'].forEach(function(name) {
                        video.addEventListener(name, function() {
                            ...
                            if (name === 'play') window.__cluUserPaused = false;
                            if (name === 'pause') {
                                // Provider pages often pause on
                                // visibilitychange when the screen turns off.
                                // With background play ON and no explicit user
                                // pause, resume so audio continues.
                                if (document.hidden) {
                    ...
                    // Late registration: a video mounted while already hidden
                    // (screen off) still gets the resume treatment.
                    if (document.hidden) {
                        setTimeout(function() { maybeResumeAfterHide(video); }, 500);
                    }
                ...
                window.__cluVisibilityListener = function() {
