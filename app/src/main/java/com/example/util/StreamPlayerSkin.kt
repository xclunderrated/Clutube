...
internal object StreamPlayerSkin {
    ...
    fun interceptPlayerJs(request: WebResourceRequest?): WebResourceResponse? {
        ...
        cachedAsset(requestUrl)?.let {
            return WebResourceResponse(
                "application/javascript", "UTF-8", java.io.ByteArrayInputStream(it)
            )
        }
    ...
    fun interceptPlayerCss(request: WebResourceRequest?): WebResourceResponse? {
        ...
        cachedAsset(requestUrl)?.let {
            return WebResourceResponse("text/css", "UTF-8", java.io.ByteArrayInputStream(it))
        }
        ...
        return try {
            ...
            putCachedAsset(requestUrl, combinedCss.toByteArray(Charsets.UTF_8))
            WebResourceResponse(
                "text/css",
                "UTF-8",
...
(function () {
    ...
    if (window.MutationObserver && document.documentElement) {
        // Dock only needs to run when player chrome mounts; throttle to
        // avoid re-layout on every DOM mutation during playback.
        var __cluDockLast = 0;
        new MutationObserver(function() {
            var now = Date.now();
            if (now - __cluDockLast < 1500) return;
            __cluDockLast = now;
            dockPlayerActions();
...
(function () {
    ...
    window.__cluPlayerPreferenceListener = function (event) {
        ...
        // Re-arm the retry loop: it stops itself once applied, so a later
        // change arriving while the player is mid-mount needs it restarted.
        if ((pendingQuality !== null || pendingSubtitles !== null) && !window.__cluPreferenceRetry) {
            startPreferenceRetry();
        }
    };
    window.addEventListener('message', window.__cluPlayerPreferenceListener);

    function startPreferenceRetry() {
        ...
        window.__cluPreferenceRetry = setInterval(function () {
            ...
            // Stop polling once applied: the old 250ms-forever retry woke
            // the renderer constantly during playback (jank).
            if (pendingQuality === null && pendingSubtitles === null && window.__cluPreferenceRetry) {
                clearInterval(window.__cluPreferenceRetry);
                window.__cluPreferenceRetry = null;
            }
        }, 1000);
    }
    startPreferenceRetry();
...
(function () {
    ...
    if (window.MutationObserver && document.documentElement) {
        ...
        new MutationObserver(__cluThrottledReport).observe(document.documentElement, {
            attributes: true,
